...
const LOCAL_FAVORITES_KEY = 'SAGA_FAVORITES'; // ⭐ mismos favoritos que en detalle/catálogo personalizado
...
const lastFavoriteChange = ref(localStorage.getItem('favoritesChanged') || null);
...
const checkFavoriteStatus = async () => {
  // 1️⃣ Baseline desde localStorage (para cuando NO hay token o antes de que responda el backend)
  const storedIds = loadFavoritesFromStorage();
  const baseStatuses = {};

  productos.value.forEach(p => {
    if (!p || p.idproducto == null) return;
    const pid = parseInt(p.idproducto, 10);
    baseStatuses[pid] = storedIds.includes(pid);
  });

  favoriteStatuses.value = baseStatuses;

  // 2️⃣ Si no hay token, solo usamos localStorage
  ...
  try {
    ...
    // ❗️AQUÍ EL CAMBIO IMPORTANTE:
    // SI EL BACKEND DICE "NO HAY FAVORITOS", ENTONCES TODO DEBE IR A FALSE

    const serverStatuses = {};
    productos.value.forEach(p => {
      if (!p || p.idproducto == null) return;
      const pid = parseInt(p.idproducto, 10);
      serverStatuses[pid] = favoriteIds.includes(pid); // si favoriteIds está vacío → todos false
    });

    favoriteStatuses.value = serverStatuses;

    // Sincronizamos localStorage con lo que diga el backend (aunque esté vacío)
    saveFavoritesToStorage(favoriteIds); // si está vacío, borramos todos del storage

  } catch (error) {
    console.error("Error al verificar el estado de favoritos:", error);
    // No tocamos favoriteStatuses en error para no parpadear la UI
  }
};


// 🔄 Sincronizar favoritos cuando cambien en otra vista/pestaña
const syncFavoritesFromStorage = async () => {
  const currentChangeTime = localStorage.getItem('favoritesChanged');

  // Si hay un cambio nuevo, recargamos solo el estado de favoritos
  if (currentChangeTime && currentChangeTime !== lastFavoriteChange.value) {
    lastFavoriteChange.value = currentChangeTime;
    await checkFavoriteStatus();  // 🔁 vuelve a marcar verde/gris según backend
  }
};

// Escuchamos el evento 'storage' para cambios en OTRAS pestañas
const handleStorage = (event) => {
  if (event.key === 'favoritesChanged') {
...
const toggleFavorite = async (producto) => {
  ...
  const currentStatus = !!favoriteStatuses.value[pid];

  // Optimistic UI
  const newStatuses = { ...favoriteStatuses.value };
  newStatuses[pid] = !currentStatus;
  favoriteStatuses.value = newStatuses;

  const nextStatus = newStatuses[pid];
  ...
  try {
    ...
    // Si backend manda isFavorite, lo respetamos
    if (response.data && typeof response.data.isFavorite === 'boolean') {
      const finalStatuses = { ...favoriteStatuses.value };
      finalStatuses[pid] = response.data.isFavorite;
      favoriteStatuses.value = finalStatuses;
    }

    // Actualizar localStorage en función del estado final (nextStatus)
    ...
    // Notificar a otras vistas que algo cambió
    ...
    revertStatuses[pid] = currentStatus;
...
const fetchProductos = async () => {
  ...
  try {
    ...
    // Después de cargar productos, sincronizamos favoritos
...
onMounted(() => {
    ...
    // 🔔 Cuando la pestaña reciba foco, sincronizamos
    window.addEventListener('focus', syncFavoritesFromStorage);

    // 🔔 Cuando otra pestaña cambie favoritesChanged, nos enteramos
...
// 🔁 Cuando vuelves a esta vista (por ejemplo, desde Favoritos)
onActivated(async () => {
  // Recalcular productos y, dentro de fetchProductos, se llamará a checkFavoriteStatus()
